...
function Column (props) {
  ...
  return (
    <g className="chart-bar">
      ...
      <line className="chart-bar__separator" x1={left} y1={top} x2={left} y2={bottom} stroke="#4984af"/>
      <text className="chart-bar__label" textAnchor="middle" x={left + width / 2} y={top + labelHeight} fill={labelColor}>{label}</text>
    </g>
  );
}
...
export default class LineChart extends React.Component {
  ...
  render() {
    ...
    // calculated
    let graphMaxY = Math.max.apply(Math,dataPoints.map(function(o){return o.y;}));
    let graphMinY = Math.min.apply(Math,dataPoints.map(function(o){return o.y;}));
    let graphDiff = graphMaxY - graphMinY;
    let graphScaleY = (graphHeight - strokeWallHeight - graphPaddingTop - graphPaddingBottom) / graphDiff;
    ...
    for (let i = 0; i < dataPoints.length; i++) {
      ...
      let command = (i === 0) ? "M" : "L";
      graph += command + (graphX + barWidth * i + barWidth / 2) + " " + (graphLineY - (y - graphMinY) * graphScaleY);
    }
    ...
    for (let i = 0; i < dataPoints.length; i++) {
      let y = dataPoints[i].y;
      let command = (i === 0) ? "M" : "L";
      blurGraph += command + (graphX + barWidth * i + barWidth / 2) + " " + (graphLineY + strokeWallHeight - (y - graphMinY) * graphScaleY);
    }
    ...
    for (let i = 0; i < dataPoints.length; i++) {
      let y = dataPoints[i].y;
      let command = (i === 0) ? "M" : "L";
      dropShadowGraph += command + (graphX + barWidth * i + barWidth / 2) + " " + (graphLineY + strokeWallHeight - (y - graphMinY) * graphScaleY);
    }
    ...
    let dropShadowStartHeight = (graphY + graphHeight)-(graphLineY - (dataPoints[0].y - graphMinY) * graphScaleY);
    let dropShadowEndHeight = (graphY + graphHeight)-(graphLineY - (dataPoints[dataPoints.length-1].y - graphMinY) * graphScaleY);

    let dropShadowStartShift = (dropShadowStartHeight / Math.sin(toRadians(dropShadowCAngle))) * Math.sin(toRadians(dropShadowAAngle));
    let dropShadowEndShift = (dropShadowEndHeight / Math.sin(toRadians(dropShadowCAngle))) * Math.sin(toRadians(dropShadowAAngle));

    if (dropShadowEndShift > barWidth / 2) {
      let dropShadowBarY = (barWidth / 2 / Math.sin(toRadians(dropShadowAAngle))) * Math.sin(toRadians(dropShadowCAngle));

      dropShadowGraph += "L" + (graphX + graphWidth) + " " + (graphY + graphHeight - dropShadowEndHeight + dropShadowBarY);
      dropShadowGraph += "L" + (graphX + graphWidth) + " " + (graphY + graphHeight);
    } else {
      dropShadowGraph += "L" + (graphX + graphWidth - barWidth / 2 + dropShadowEndShift) + " " + (graphY + graphHeight);
    }
    dropShadowGraph += "L" + (graphX + barWidth / 2 + dropShadowStartShift) + " " + (graphY + graphHeight) + "Z";

    let wallGraph = "";
    for (let i = 0; i < dataPoints.length; i++) {
      let y = dataPoints[i].y;
      let command = (i === 0) ? "M" : "L";
      wallGraph += command + (graphX + barWidth * i + barWidth / 2) + " " + (graphLineY - (y - graphMinY) * graphScaleY);
    }
    let reversed = dataPoints.reverse();
    for (let i = 0; i < reversed.length; i++) {
      let y = reversed[i].y;
      wallGraph += "L" + (graphX + barWidth * (reversed.length - i - 1) + barWidth / 2) + " " + (graphLineY + strokeWallHeight - (y - graphMinY) * graphScaleY);
    }
    wallGraph += "Z";
    ...
    return (
      <svg className={"line-chart line-chart--" + lineClassModifier} width={width} height={height} xmlns="http://www.w3.org/2000/svg">
        ...
        <rect fill="#65aadc" x={graphX} y={graphY} width={graphWidth} height={graphHeight}/>
